...
function renderizar() {
    ...
    for (let i = 1; i < cobra.length - 1; i++) {
...
function gerarComida() {
    ...
    while (!posicaoLivre) {
        ...
        for (let i = 0; i < cobra.length; i++) {
            if (cobra[i].x === novaComida.x && cobra[i].y) {
                posicaoLivre = false;
                break;
            }
        }
...
function mudarDirecao(tecla) {
    ...
    else if (tecla === 'ArrowDown' && direcao.y !== -1) {
        proximaDirecao = {x:0, y:1};
    }
    else if (tecla === 'ArrowLeft' && direcao.x !== 1) {
        proximaDirecao = {x:-1, y:0};
    }
    else if (tecla === 'ArrowRight' && direcao.x !== -1) {
        proximaDirecao = {x:1, y:0};
    }
}
...
function encerrarJogo() {
    ...
    document.getElementById('overlay').classList.add('visível');
}
